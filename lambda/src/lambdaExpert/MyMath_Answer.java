...
//    	처음 수가 음수인 경우와 아닌 경우로 나눠서 로직 동작
    		  if(expression.charAt(0) == '-') {				// 처음 수가 음수라면 배열의 값이 {"", 정수, 정수}이렇게 돼서 
    			  for(int i = 0; i < strResult.length; i++) {
    				  strResult[i] = tmp[i+1];				// 결과배열에 공백을 제외한 정수만 담기 위해, 문자열 배열 인덱스 +1 저장	
    			  }
    		  } else {										// 처음 수가 양수라면
...
//		연산자만 있는 배열이 필요해서 임의로 만든 메서드
      operArr = new MyMath_Answer().getOperArr(inputString);
...
// 위에서 만든 모든 메서드들 사용
      for(int i = 0; i < nonOpperArr.length; i++) {					// 연산자 제외한 정수만 갖고 있는 문자열 배열을 순회하며
    	  if(inputString.charAt(0) != '-') {						// 입력값의 첫번째 인덱스가 -가 아니면서, 						(위에서 '-' 비교한건 뭐지??? --> getOpers와 getOperArr를 합쳐서 계산할 때 제일 앞에 음수 기호가 붙으면 배열인덱스가 안맞아져서 분기를 둔 것. 근데 메서드 선언에서 완벽히 처리했으면 여기서 또 조건 안줘도 됨.)
    		  if(i == 0) {											// i가 0일 때만 (i는 그냥 숫자일 뿐이지 않나??? 이거를 왜 조건으로 준거지 ??? --> i 초기값 없이 preData = Integer.parseInt(nonOpperArr[i])를 반복실행하면 결과가 누적 되지 않고 덮저씌어버리기 때문
    		  ...
    		  result = Integer.parseInt(nonOpperArr[i + 1]);		// +1은 왜 하는거지????? --> 현재 i번째 숫자는 이미 preData로 썼으니까, 다음 숫자(i+1)부터 연산을 해야 하기 때문에 +1을 한다.
    	  } else {
    	  ...
    	  System.out.println("결과 값\t:"+ preData);
