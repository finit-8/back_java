...
//      6. "Apple", "banana", "Melon", "orange" 중 앞글자가 대문자인 문자열만 출력
	   ...
	   fruites.stream()
	   	.filter((c) -> c.charAt(0) >= 'A' && c.charAt(0) <= 'Z')
	   	.forEach(System.out::println);
	   
//      7. 한글을 정수로 변경 "공칠이삼" -> 0723
	   String hangle = "공일이삼사오육칠팔구";
	   String input = "공칠이삼";
	   input.chars().map(c -> hangle.indexOf(c)).forEach(System.out::println);
...
//	   String input = "공칠이삼";
//	   String hangle = "공일이삼사오육칠팔구";
//	   input.chars().forEach((e) -> {System.out.println((char)e);});		// 인덱스 하나하나 순회
//	   input.chars().map((e) -> hangle.indexOf(e)).forEach(System.out::println);
//	   input.chars().map(hangle::indexOf).forEach(System.out::println);
//
//	   // 8.
//	   String input2 = "0723";
//	   input2.chars().map((c) -> c - (int)'0').map(hangle::charAt).mapToObj((c) -> (char)c).forEach(System.out::println);
//	   
//	   
